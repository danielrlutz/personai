...
**Drive folder matching:** PersonAI does **not** invent a second taxonomy next to yours. Under your archive root (`Archived Files`, `PersonAI_Archive`, or similar), it matches category folders by name — numbering style (`01_`, `1.`, `01 -`, …) and EN/DE/FR labels (e.g. `1. Official Documents` ↔ Official, `Versicherung` ↔ Insurance). If regex is unsure, a small local Ollama model may pick once; the mapping is cached per profile so uploads reuse it. New `01_Label` folders are created only when nothing matches. PersonAI **never silent-deletes** Drive folders — if you already have both `01_Official` and `1. Official Documents`, use **Combine folders** in Settings (dry-run + explicit confirms) or merge in Google Drive; Prefer forever only caches the winner for uploads.

**Drive knowledge:** PersonAI can index **everything** under your configured Drive root into a **private local** retrieval store (`drive-knowledge.db` under the profile data directory). Hybrid keyword search (SQLite FTS5) plus optional local Ollama embeddings (e.g. `nomic-embed-text`) — **not** Google Gemini API. OCR text from already-ingested local documents is attached when filenames match. Team specialists, Staff triage, and the morning brief receive char-budgeted top-k snippets (with Drive file ids for View file). Filenames teach naming muscle memory (Invoice/Rechnung → BILL, entity tokens, date patterns, folder numbering).

How to reindex: **Settings → Google Drive → Reindex Drive knowledge** (requires a root folder ID). **Refresh archive context** also queues a full reindex after the compact taxonomy snapshot. Status shows file/chunk counts and whether embeddings are active (keyword-only until an embed model is pulled).
...
**Combine folders (Settings → Google Drive archive):** Manually multi-select taxonomy folders (e.g. `01_Official` + `1. Official Documents`), choose a **destination** (kept name/id), then run a **dry-run preview**. Conflicts show both sides (name, size, modified, folder) with Explorer-style choices: Keep destination, Keep incoming (renames the existing file aside), Keep both (auto-rename), or Skip. Combine **never silent-deletes** — files are moved via Drive parents; trashing an existing conflict item or removing an empty source folder requires an explicit per-item / second confirm. **Confirm combine** stays disabled until you mark the dry-run reviewed; long runs use a durable server job.
