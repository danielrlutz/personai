...
write_env() {
  ...
  # Bundled compose Ollama is opt-in via COMPOSE_PROFILES=bundled-ollama.
  ...
  if [[ $IS_UPDATE -eq 1 && -f "$env_file" ]]; then
    ...
    set_kv COMPOSE_PROFILES "$compose_profiles"
    mv "$tmp" "$env_file"
...
PERSONAI_OLLAMA_MODE=${OLLAMA_MODE}
# Empty = use host/native Ollama (no docker ollama service). Set bundled-ollama to opt in.
...
write_compose_override() {
...

  {
    ...
    echo "# Base compose keeps ollama behind profile bundled-ollama (off by default)."
    echo "services:"
    if [[ "$OLLAMA_MODE" == "new-docker" ]]; then
      echo "  ollama:"
      echo "    ports:"
...
start_stack() {
  ...
  if [[ "$OLLAMA_MODE" == "new-docker" ]]; then
    info "Profile bundled-ollama enabled (compose Ollama)"
    COMPOSE_PROFILES="${COMPOSE_PROFILES:+$COMPOSE_PROFILES,}bundled-ollama" \
      docker compose "${files[@]}" --profile bundled-ollama up -d --build
  else
    info "Starting without docker Ollama (host/native or skip) — no :11434 bind"
    # Force empty profile so a stale shell/env COMPOSE_PROFILES cannot start ollama
    COMPOSE_PROFILES= docker compose "${files[@]}" up -d --build
  fi
  ok "Stack started"
}
...
pull_models() {
  ...
  if [[ "$OLLAMA_MODE" == "new-docker" ]]; then
    local -a files=()
    while IFS= read -r line; do files+=("$line"); done < <(compose_files)
    docker compose "${files[@]}" --profile bundled-ollama exec -T ollama ollama pull maternion/LightOnOCR-2 || true
    docker compose "${files[@]}" --profile bundled-ollama exec -T ollama ollama pull deepseek-r1:8b || true
...
install_systemd_user() {
  ...
  local profile_flag=""
  [[ "$OLLAMA_MODE" == "new-docker" ]] && profile_flag="--profile bundled-ollama "
  mkdir -p "$unit_dir"
...
# COMPOSE_PROFILES from .env selects bundled-ollama when using compose Ollama
ExecStart=/usr/bin/docker compose -f docker-compose.yml -f docker-compose.override.yml ${profile_flag}up -d
...
run_install() {
  ...
  if [[ "$START_NOW" == "yes" ]]; then
    ...
    if [[ "$OLLAMA_MODE" == "new-docker" ]]; then
      info "Skipping start. Later: cd $INSTALL_DIR && docker compose --profile bundled-ollama up -d --build"
    else
