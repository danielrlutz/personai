...
write_env() {
  ...
  if [[ $IS_UPDATE -eq 1 && -f "$env_file" ]]; then
    ...
    # COMPOSE_FILE in .env would pull docker-compose.ollama.yml into every bare `docker compose`
    if grep -qE '^COMPOSE_FILE=' "$tmp" 2>/dev/null; then
      awk 'BEGIN{FS=OFS="="} $1=="COMPOSE_FILE"{next} {print}' "$tmp" >"${tmp}.new"
      mv "${tmp}.new" "$tmp"
    fi
    mv "$tmp" "$env_file"
    ok "Merged settings into existing .env (data preserved; COMPOSE_PROFILES/COMPOSE_FILE cleared)"
    return
  fi
...
PERSONAI_OLLAMA_MODE=${OLLAMA_MODE}
# Legacy keys — leave empty / unset. Bundled Ollama: -f docker-compose.ollama.yml only.
COMPOSE_PROFILES=
# Do not set COMPOSE_FILE= here (would force the ollama overlay on every compose).
...
write_compose_override() {
...

  {
    ...
    echo "# This override must NOT define a standalone ollama service when using host Ollama."
    echo "services:"
    # GPU hints only when bundled compose Ollama is explicitly selected.
    # Never emit ollama: for existing-native / existing-docker / skip — stale
    # override blocks from older installers caused :11434 bind conflicts.
    if [[ "$OLLAMA_MODE" == "new-docker" ]]; then
...
remove_stale_ollama_container() {
  ...
  COMPOSE_FILE= COMPOSE_PROFILES= docker compose "${files[@]}" rm -f -s ollama 2>/dev/null || true
  local base
...
start_stack() {
  ...
  # Always clear COMPOSE_FILE / COMPOSE_PROFILES — stale env must never pull ollama overlay
  unset COMPOSE_FILE || true
  export COMPOSE_FILE=
  export COMPOSE_PROFILES=
  if [[ "$OLLAMA_MODE" == "new-docker" ]]; then
    info "Starting with -f docker-compose.ollama.yml (compose Ollama)"
    COMPOSE_FILE= COMPOSE_PROFILES= docker compose "${files[@]}" up -d --build --remove-orphans
  else
    info "Starting without docker Ollama (host/native or skip) — no :11434 bind"
    remove_stale_ollama_container
    local svc
    svc="$(COMPOSE_FILE= COMPOSE_PROFILES= docker compose "${files[@]}" config --services 2>/dev/null || true)"
    if echo "$svc" | grep -qi '^ollama$'; then
      warn "compose config still lists ollama — check COMPOSE_FILE / override; see ./scripts/vps-recover-api.sh"
    fi
    COMPOSE_FILE= COMPOSE_PROFILES= docker compose "${files[@]}" up -d --build --remove-orphans
  fi
  ok "Stack started"
