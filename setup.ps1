...
$RepoUrl = if ($env:PERSONAI_REPO_URL) { $env:PERSONAI_REPO_URL } else { 'https://github.com/YOUR_ORG/personai-os.git' }
...
$RawBase = if ($env:PERSONAI_RAW_BASE) { $env:PERSONAI_RAW_BASE } else { 'https://raw.githubusercontent.com/YOUR_ORG/personai-os/main' }
...
function Get-NativeOllamaInfo {
  ...
  if (Test-Command 'ollama') {
    $info.Binary = (Get-Command ollama -ErrorAction SilentlyContinue).Source
  } else {
  ...
  try {
    ...
    if ($listeners) { $info.Port = $true }
  } catch {
    if (netstat -ano 2>$null | Select-String ':11434\s') { $info.Port = $true }
  }

  foreach ($url in @('http://127.0.0.1:11434/api/tags', 'http://localhost:11434/api/tags')) {
    try {
      $null = Invoke-WebRequest -Uri $url -UseBasicParsing -TimeoutSec 2
      $info.HttpOk = $true
      $info.Host = ($url -replace '/api/tags$', '')
      break
    } catch {}
  }
...
function Test-NativeOllamaDetected {
  $i = Get-NativeOllamaInfo
  return [bool]($i.Binary -or $i.Process -or $i.Service -or $i.Port -or $i.HttpOk)
}

function Test-OllamaOk {
  $i = Get-NativeOllamaInfo
  if ($i.HttpOk) {
    Write-Ok "Native Ollama HTTP at $($i.Host) (/api/tags)"
    return $true
  }
  if ($i.Binary) {
    Write-Ok "Ollama CLI: $($i.Binary) (API not answering yet on :11434)"
    return $true
  }
  if ($i.Process -or $i.Service -or $i.Port) {
    Write-Ok 'Ollama process/service/port signals present'
    return $true
  }
  Write-Warn 'Native Ollama not detected (optional for desktop — https://ollama.com)'
  return $false
}

function Ensure-DesktopOllamaEnv {
  ...
  if (-not (Test-Path $envFile)) {
    @"
# Generated by setup.ps1 — desktop / native Ollama
DATA_DIR=./data
PORT=4000
OLLAMA_HOST=$HostUrl
OLLAMA_PUBLIC_HOST=$HostUrl
...
"@ | Set-Content -Path $envFile -Encoding UTF8
    Write-Ok "Wrote $envFile (native Ollama ? $HostUrl)"
    return
  }
  $text = Get-Content -Path $envFile -Raw -ErrorAction SilentlyContinue
  if ($text -match '(?m)^OLLAMA_HOST=') {
    Write-Info 'Keeping existing OLLAMA_HOST in .env'
  } else {
    Add-Content -Path $envFile -Value "`nOLLAMA_HOST=$HostUrl`nPERSONAI_OLLAMA_MODE=existing-native"
    Write-Ok "Appended OLLAMA_HOST=$HostUrl to .env"
  }
}
...
function Invoke-PullModels {
  Set-Location $script:InstallDir
  $native = Get-NativeOllamaInfo
  $doPull = $PullModels
  if ($doPull -eq 'ask') {
    $defaultPull = if (Test-NativeOllamaDetected) { 'y' } else { 'n' }
    $doPull = if (Read-YesNo 'Pull Ollama models (OCR + reasoning)?' $defaultPull) { 'yes' } else { 'no' }
  }
  if ($doPull -ne 'yes') { return }

  # Prefer native Windows Ollama — do not require Docker Desktop for desktop mode.
  $cli = $null
  if ($native.Binary) { $cli = $native.Binary }
  elseif (Test-Command 'ollama') { $cli = 'ollama' }

  if ($cli) {
    Write-Info 'Pulling models via native Ollama...'
    if (-not $native.HttpOk) {
      Write-Info 'Starting ollama serve in background...'
      Start-Process -FilePath $cli -ArgumentList 'serve' -WindowStyle Hidden
      Start-Sleep -Seconds 2
    }
    & $cli pull maternion/LightOnOCR-2
    & $cli pull deepseek-r1:8b
  } elseif ((Test-Command 'docker') -and (docker compose ps ollama 2>$null)) {
    ...
    Write-Warn 'Neither native ollama.exe nor compose ollama available - skip model pull'
    return
  }
...
function Invoke-DesktopMode {
  ...
  $nativeEarly = Test-NativeOllamaDetected
  if ($isFull) { Initialize-Steps 11 }
  elseif ($nativeEarly) { Initialize-Steps 10 }
  else { Initialize-Steps 9 }

  Write-Step 'Resolve repository'
  Resolve-RepoDir

  Write-Step 'Native Ollama'
  $native = Get-NativeOllamaInfo
  if (Test-NativeOllamaDetected) {
    Write-Ok "Using existing native Ollama at $($native.Host)"
    if ($native.Binary) { Write-Info "CLI: $($native.Binary)" }
    if ($native.Service) { Write-Info 'Windows service Ollama is running' }
    Ensure-DesktopOllamaEnv -HostUrl $native.Host
  } else {
    Write-Warn 'No native Ollama detected — desktop can still run; install later from https://ollama.com'
    Write-Info 'Docker Desktop is optional and not required for a host-installed Ollama.'
  }
  ...
  if ($isFull) {
    Write-Step 'Docker (optional — not required for native Ollama)'
    $wantDocker = $Docker
    if ($wantDocker -eq 'ask') {
      # Default no: desktop prefers native Ollama; Docker is for VPS/bundled path.
      $wantDocker = if (Read-YesNo 'Ensure Docker Desktop (VPS / bundled Ollama path)?' 'n') { 'yes' } else { 'no' }
    }
    if ($wantDocker -eq 'yes') { Install-DockerWin } else { Write-Info 'Skipping Docker (native Ollama / desktop path)' }
  }
  ...
  if ($isFull -or (Test-NativeOllamaDetected)) {
...
function Show-DesktopSummary {
  ...
  $ollama = Get-NativeOllamaInfo
  $ollamaLine = if (Test-NativeOllamaDetected) { "native $($ollama.Host)" } else { 'not detected' }
  ...
  Write-Host ""
  Write-Host 'Native Ollama (default for desktop): OLLAMA_HOST=http://127.0.0.1:11434 in .env' -ForegroundColor DarkGray
