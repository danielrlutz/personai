...
export function ExportBuilder() {
  ...
  const queueExport = async () => {
    ...
    try {
      ...
      // Legacy path if server returns PDF immediately
    ...
    <div className="space-y-3">
      <ConfirmGate
        ...
        onResolved={(d) => {
          if (d === "confirm") {
            setAwaiting(false);
            setDonePath("Medical PDF saved under this profile’s exports folder.");
          }
        ...
        <CardContent className="space-y-4">
          ...
          {awaiting && (
            <p className="text-sm text-muted-foreground">
              Waiting for confirmation — approve above to write the PDF (
              {countLabel(selected.size, "symptom entry", "symptom entries")}).
            </p>
          )}
          {donePath && <p className="text-sm text-primary">{donePath}</p>}
          {error && <p className="text-sm text-red-400">{error}</p>}

          <Button onClick={() => void queueExport()} disabled={loading || selected.size === 0 || awaiting}>
            {loading ? "Queuing…" : awaiting ? "Waiting for confirmation…" : "Prepare export"}
          </Button>
        </CardContent>
      </Card>
    </div>
